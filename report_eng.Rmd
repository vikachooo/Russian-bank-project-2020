---
title: "Solvency Scapes: Navigating Russia's Regional Economies for Banking Growth"
...
setwd("/Users/viktoriazajceva/Desktop/R/20 Проект/FINAL")
```
...
data <- read.csv("All_data_eng.csv")  # Read data without converting strings to factors
...
```{r ggplot Dynamics of SberIndex indicators in 2020 by month, warning=FALSE, message=FALSE, echo=FALSE, results='hide'}
...
```{r ggplot tourism and online transactions, warning=FALSE, message=FALSE, echo=FALSE, results='hide'}
...
```{r shiny1 for indicators, echo=FALSE}

# upload data again just in case
all_data_eng <- read.csv("All_data_eng.csv", fileEncoding = "UTF-8")
...
all_data_eng_col <- read.csv("All_data_eng_columns.csv")
all_data_eng_col <- all_data_eng_col[,-1]
...
##### map_data ####
map_data <- all_data_eng_col[,c(1,3,5,6,8,9)]
